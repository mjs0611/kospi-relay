...
## 5차: 밤 / 낮 분할, 호가창식 막대 (2026-09-07 저녁)

"디자인이 너무 AI스럽다." 맞다. 검은 바탕 + 보라·틸 오로라, 유리 테두리, 알약 막대, 모노 숫자, 스펙트럼 선,
그라디언트 버튼은 frontend-design 스킬이 AI 기본값으로 꼽는 목록의 합이었다. 주제에서 형태를 꺼내 다시 잡는다.

**형태.** 카드를 **밤 / 낮**으로 반 가른다. 왼쪽(#15171D)은 밤사이 뉴욕, 오른쪽(#EEF0F3)은 다음 날 코스피 시가.
경계가 곧 릴레이. 조건("뉴욕이 크게 오른 밤")은 밤에, 규칙("다음 날 코스피는 10번 중 8번 위로 열렸다")은 낮에 산다.
막대는 한국 증권 앱의 **호가창** 방식: 행마다 가로 막대(길이 = 크기, 색 = 방향), 오늘이 속한 행을 옅게 칠하고
끝에 "오늘 +3.34%". 뉴욕 쪽도 같은 행 구조.

**뺀 것.** 오로라·유리·글로우·그림자·그라디언트·스펙트럼 선·모노 폰트(Azeret Mono 링크 제거)·알약 라운드.
채도는 상승 빨강·하락 파랑뿐. 라운드 10/8px. Pretendard 하나, 숫자는 `font-variant-numeric: tabular-nums`.
미니앱은 라이트로(color-scheme light, 배너 theme light), 버튼·칩은 플랫 #15171D.

**구현.** 쪽마다 CSS 변수 스코프(`.night` / `.day`)를 둬 같은 SVG(`ny_svg` 236×84, `kr_svg` 320×100)가 밤·낮 색으로
렌더된다. TDS 버튼 표면색은 선택자 우선순위로 못 이겨 `!important`(실측). 폰(<600px)에선 두 쪽이 위아래로.

## 말투 (2026-09-07 저녁)

"열렸다" 같은 신문체가 AI스럽다. 폰으로 보는 말이 아니다. fx-signal("환전하기 좋아요", "지금은 비싼 편")과
같은 목소리로: **해요체, 짧게, 구어.** 행 이름은 증권 앱 표준어 상승 / 보합 / 하락.
- 밤: "지난밤 뉴욕이 크게 올랐어요 / 조금 올랐어요 / 잠잠했어요 / 조금 내렸어요 / 크게 내렸어요"
- 낮: "코스피는 10번 중 8번 올라서 시작했어요" (내려서 / 거의 그대로)
- 대기: "오늘 시가는 9시에 나와요", 휴장: "오늘은 휴장이에요", 표본 부족: "비슷한 밤이 12번뿐이라 통계는 안 냈어요"
- 캡션: "2021년부터 비슷한 밤 278번"
테스트가 열렸다·습니다·줄표·중점을 막는다.

## 스케줄·텔레그램 (2026-09-08)

GitHub cron은 살아 있지만 9/7 실측으로 4~5시간씩 늦게 돈다(09:09 예정 → 13:22 실행). 아침 습관 앱에 못 쓴다.
정시 트리거는 **사용자 Mac의 launchd**가 맡는다: `ops/com.junseungmo.kospi-relay.plist`가 평일 06:45·09:06에
`ops/dispatch.sh`를 실행하고, 스크립트가 시각으로 단계를 정해(8시 전 morning, 이후 open) `gh workflow run`을 쏜다.
Mac이 잠들어 있었으면 깨어난 직후 한 번 실행된다. 꺼져 있으면 GitHub cron(백업)이 늦게라도 돈다.
로그 `~/Library/Logs/kospi-relay.log`.

텔레그램 발송은 제거. 시크릿이 한 번도 없었고 사용자가 안 쓴다. 미결 항목 "텔레그램 채널 개설" 종결.

## 낮 동인 (2026-09-08)

"낮에 찾아올 동인이 적다." 맞다. 루프는 아침 두 박자뿐이었고, Constraints(시가까지만, 인트라데이 금지)가
장중 갱신을 막는다. 그 제약 안에서 둘을 넣는다. 알림은 앱인토스 정책 부담으로 보류.
- **15:40 마감 박자(`close` 단계).** 오늘 종가(일별)로 "오늘 코스피는 +1.24%로 마감했어요. 뉴욕은 22:30에 열려요"
  한 줄. 오늘을 닫고 오늘 밤을 예고한다. status `done`, JSON `close`·`tail`. 장중 야후 일봉의 Close는 현재가라
  close 단계에서만 믿고, 종가 반영이 늦을 수 있어 16:10 재시도. launchd 15:40, GitHub cron 15:40·16:10 백업.
- **지난 밤들 칩 색 점.** `index.json`을 `[{d, z}]`(그날 시가 방향)로. 칩에 상승 빨강·보합 회색·하락 파랑 점.
  훑어보면 어떤 밤이었는지 보인다. 앱은 옛 문자열 배열도 받는다.
루프: 06:45 질문 → 09:06 답 → 15:40 마감·예고. 세션 수가 아니라 아침 열람률이 지표라는 판단은 유지.

## 공유와 OG (2026-09-08)

공유는 카드가 미리보기로 떠야 의미가 있다. 세 겹으로.
- **웹 날짜 페이지**: og:title=조건("지난밤 뉴욕이 크게 올랐어요"), og:description=규칙, og:image=그날 `relay.png`
  (build은 루트 PNG를 날짜 폴더에 복사, backfill은 날짜별 스크린샷). 루트 페이지는 루트 PNG. `twitter:card` 큰 이미지.
  푸터 '공유' 버튼은 Web Share, 없으면 링크 복사.
- **미니앱**: `getTossShareLink('intoss://kospi-relay/d/날짜', 그날 PNG URL)`로 토스 공유 링크에 카드를 OG로 붙인다
  (fx-signal 선례). 문장은 "조건. 규칙". share() 실패 시 클립보드에 넣고 안내.
- 딥링크 `/d/YYYY-MM-DD`는 이미 있었다. 공유받은 사람은 그 날짜 카드로 바로 온다.
